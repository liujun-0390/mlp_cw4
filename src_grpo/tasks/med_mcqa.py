...
class CustomTask(BaseTask):
    ...
    def cal_correct(self, preds, labels, metric_weights, data_type = "str"):
        '''
        <task specific>
        ...
        '''
        acc_weight, bleu_weight, rouge_weight, meteor_weight = metric_weights
        ...
        for p, l in zip(preds, labels):
            ...
            scores.append((acc_weight * acc) + (bleu_weight * bleu) + (rouge_weight * rouge) + (meteor_weight * meteor))

        return scores
    
    def cal_metric(self, preds, labels, metric_weights, questions=None):
        '''
        <task specific>
        ...
        '''
        scores = self.cal_correct(preds=preds, labels=labels, metric_weights=metric_weights)
        return np.mean(scores)

    def cal_correct_test(self, preds, labels, metric_weights, data_type = "str"):
        '''
        <task specific>
        The function of comparing the predictions and labels.
    ...
    def cal_metric_test(self, preds, labels, metric_weights, questions=None):
        '''
        <task specific>
        Calculate the evaluation metric, e.g. Accuracy, F1 score.
        "question" is for NCBI calculating F1 score.
        ...
        This function is for calculating the reward of MCTS.
        '''
        scores, all_acc, all_bleu, all_rouge, all_meteor = self.cal_correct_test(preds=preds, labels=labels, metric_weights=metric_weights)
