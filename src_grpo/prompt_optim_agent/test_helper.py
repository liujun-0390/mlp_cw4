...
#            raise ValueError(f"prompt_file path doesn't exist: {prompt_file}")
    ...
    for batch in pbar:
        ...
        metric = task.cal_metric(all_preds, all_labels, metric_weights, all_questions)
...
#                'gt_answer':batch['answer'][i],
    ...
    metric, acc, bleu, rouge, meteor = task.cal_metric_test(all_preds, all_labels, metric_weights, all_questions)
    logger.info('--------------------------------------------')
    if not isinstance(metric, tuple):
        logger.info(f"Test Metric: {metric:.4f}")
