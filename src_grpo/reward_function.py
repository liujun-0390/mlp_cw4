...
def reward_func(questions, prompt, eval_dataloader, eval_model, metric_weights):
    ...
    for batch in pbar:
        ...
        metric = _cal_metric(all_preds, all_labels, rouge_screr, metric_weights)
...
def _cal_correct(preds, labels, rouge_screr, metric_weights):
    '''
    <task specific>
...
def _cal_metric(preds, labels, rouge_screr, metric_weights):
    '''
    <task specific>
    ...
    '''
    scores = _cal_correct(preds=preds, labels=labels, rouge_screr=rouge_screr, metric_weights=metric_weights)
